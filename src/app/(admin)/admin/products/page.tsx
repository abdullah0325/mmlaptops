...
export default function ProductsPage() {
  ...
  async function deleteProduct(id: string) {
    ...
    try {
      const res = await fetch(`/api/admin/products/${id}`, { method: "DELETE", next: { revalidate: 0 } });
      if (res.ok) {
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="mb-4 md:mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <h1 className="text-xl md:text-2xl font-bold text-[#0a0a0a]">Products</h1>
        <Link
      ...
      <div className="mb-3 md:mb-4 relative">
        ...
        <input
          ...
          className="w-full rounded-lg border border-gray-300 pl-9 pr-3 py-1.5 sm:pl-10 sm:pr-4 sm:py-2 text-sm focus:ring-2 focus:ring-[#d8a928] focus:border-transparent"
        />
      </div>
              ...
              <tbody className="divide-y divide-gray-200">
                {products.map((product) => (
                  <tr key={product.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-2 py-2 md:px-4 md:py-3">
                      <p className="font-medium text-[#0a0a0a] text-sm truncate max-w-[120px] sm:max-w-[200px]">
                        {product.title}
                      </p>
