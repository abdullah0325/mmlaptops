...
// ─── System Prompt ─────────────────────────────────────────────────────────────
const SYSTEM_PROMPT = `
You are Zara — a smart, warm sales advisor for MM Laptop Center, a premium organic health brand from Pakistan.
...
<p style="font-size:13px; color:#f6a45d; font-weight:600; margin:0;">✔ Key benefit &nbsp;&nbsp; ✔ Another benefit</p>
...
KEY PRODUCTS:
- Gaming Gear → energy, stamina, testosterone, men's health
- Pink Premium Salt → pure minerals, cooking, daily wellness  
...
function isProductQuery(text: string) {
  return /gaming gear|salt|herbal|product|show|list|what do you|recommend|available|price|buy|order|purchase/i.test(
    text
  );
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!process.env.OPENAI_API_KEY) {
      return NextResponse.json({
        reply: "Ask me about Gaming Gear, Laptops, or our Herbal blends!",
        replyHtml: `
          ...
          <p style="font-size:14px; color:${C.text}; margin:0;">Ask me about ${hl("Gaming Gear")}, ${hl("Laptops")}, or our ${hl("Herbal Blends")} — or say <span style="color:${C.gold}; font-weight:600;">"show me products"</span> to browse!</p>`,
        draftOrder,
      });
    }
