...
export default async function AdminDashboardPage() {
  ...
  const stats = [
    ...
    { label: "Total Orders", value: orderCount, color: "#d8a928" },
    { label: "Total Inquiries", value: inquiryCount, color: "#5A5E55" },
    { label: "Collections", value: collectionCount, color: "#1E1F1C" },
  ];

  return (
    <main className="min-h-screen bg-[#fcf5e8] px-3 sm:px-6 py-6 sm:py-12">
      <div className="mx-auto max-w-7xl">
        <div className="mb-6 sm:mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <Link
            ...
            className="rounded-full bg-[#f6a45d] px-4 sm:px-5 py-2 sm:py-3 text-xs sm:text-sm font-semibold text-[#fcf5e8] hover:bg-[#d8861f]"
          >
            Add Product
          </Link>
        </div>
        ...
        <div className="grid gap-3 sm:gap-5 grid-cols-2 md:grid-cols-4">
          {stats.map((item) => (
            <div
              key={item.label}
              className="rounded-2xl sm:rounded-3xl border border-[#d8a928]/20 bg-white p-3 sm:p-6 shadow-sm"
            >
        ...
        <div className="mt-6 sm:mt-8 grid gap-4 sm:gap-6 grid-cols-1 lg:grid-cols-2">
          <div className="rounded-2xl sm:rounded-3xl border border-[#d8a928]/20 bg-white p-4 sm:p-6 shadow-sm">
            <div className="mb-3 sm:mb-4 flex items-center justify-between">
              <h2 className="text-base sm:text-lg font-semibold text-[#0a0a0a]">
                Recent Orders
              </h2>
              <Link
                href="/admin/orders"
                className="text-xs sm:text-sm text-[#f6a45d] hover:underline"
              >
                View all
              </Link>
            </div>
            {recentOrders.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-[#d8a928]/20">
                  ...
                  <tbody>
                    {recentOrders.map((order) => (
                      <tr
                        key={order.id}
                        className="border-b border-[#d8a928]/10"
                      >
            ...
            {recentInquiries.length === 0 ? (
              ...
              <div className="space-y-3">
                {recentInquiries.map((inquiry) => (
                  <div
                    key={inquiry.id}
                    className="flex items-start justify-between border-b border-[#d8a928]/10 pb-3 last:border-0"
                  >
                    <div>
